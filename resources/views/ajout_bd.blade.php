...
@endif
    ...
    <div class="container">
        ...
        <form action="{{route("test.store")}}" class="needs-validation" novalidate method="post">
            @csrf
           @method('post')
            ...
            <h3>Information du dessinateur</h3>
            <hr class="part_line"/>
            <div class="flex_div">
                <div class="select_show_hide">
                    <label for="auteur">Selectionnez l'auteur</label>
                    <select class="form_style" id="auteur" name="dessinateur_id">
                        <option value="" >Choisissez</option>
                        @foreach ($dessinateurs as $dessinateur)
                            <option value="{{$dessinateur->id}}">{{$dessinateur->personne->nom}}</option>
                        @endforeach
                    </select>
                </div>
                <span class="flex_div_item">ou</span>
            ...
            <div id="ajout_editeur" class="collapse flex_div_item">
                <div class="form-group">
                    <label>Nom de la maison d'edition</label>
                    <input type="text" name="nom_editeur" class="form_style" placeholder="Entez le nom de l'editeur">
                </div>
                <div class="form-group">
                    <label>Adresse</label>
                    <input type="text" name="adresse_editeur" class="form_style" placeholder="Entez l'adresse de l'editeur">
                </div>
            </div>
            <div class="center_block">   
                <div class="flex_div_center">
                    <button type="submit" class="btn_style flex_div_center_item">Enregistrer</button>
                    <button type="reset"class="btn_style btn_annule flex_div_center_item">Reinitialiser</button>
                </div>
            </div>
        </form>
    <div>
